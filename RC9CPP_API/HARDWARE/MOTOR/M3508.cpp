...
{
    if (abs(target_angle - pos_all) < speed_plan_end_pos && pos_speed_plan.m_finalSpeed == 0.0f)
    {
    ...
    {
        target_rpm = pos_speed_plan.plan(pos_all);
        return increPID_speed();
    }
}
...
float m3508p::get_pos_all() {return pos_all;}
...
bool m3508p::set_pos_speedplan(float target_angle_, float max_speed, float max_acc, float max_dec, float finalspeed)
{
    if(pos_speed_plan.isFinished())
    {
        ...
        if(abs((float)rpm) > min_start_rpm && (target_angle - pos_all) * (float)rpm > 0.0f)
        {
            pos_speed_plan.start_plan(max_acc, max_dec, max_speed, rpm/gear_ratio, finalspeed, pos_all, target_angle_);
            return true;
        }
        else
        {
            pos_speed_plan.start_plan(max_acc, max_dec, max_speed, min_start_rpm/gear_ratio, finalspeed, pos_all, target_angle_);
            return true;
        }
    }
    else
