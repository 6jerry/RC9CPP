...
void radar_sensor::DataReceivedCallback(const uint8_t *byteData, const float *floatData, uint8_t id, uint16_t byteCount)
{
	...
	//处理雷达数据
	if (id == 2 && byteCount == 20)
	{	
//		ros_radar_loaction.world_pos.x = - kalman[0].filter(floatData[0]);
//		ros_radar_loaction.world_pos.y = -kalman[1].filter(floatData[1]); // 把上位机坐标与追踪坐标方向对齐
...
//		ros_radar_loaction.world_pos.y = f_BFP[1].filter(floatData[0]); // 把上位机坐标与追踪坐标方向对齐
	...
	{
		ros_radar_loaction.world_pos.x = 0.0f;
		ros_radar_loaction.world_pos.y = 0.0f;
...
err_code radar_sensor::check_error(){
	static uint16_t count = 0;
	if(if_active){
		count++;
		return ERR_CODE_NOT_INITIALIZED;
	}else{
		count = 0;
...
Vector2D radar_sensor::get_world_pos(void)
{
	return real_radar_world_pos;
}

float radar_sensor::get_heading(void)
{
	return ros_radar_loaction.yaw_angle;
}

float radar_sensor::get_yaw_rad()
{
	return ros_radar_loaction.yaw_angle * 0.01745329252f;
}

// 添加重定位imu
void radar_sensor::add_recolate_imu(imu *_imu)
{
	imu_ = _imu;
}

// 设置重定位标志，用雷达校准
void radar_sensor::relocate_imu(void)
{
	relocate_flag = true;
}

// 设置重定位标志，停止用雷达校准
void radar_sensor::stop_relocate(void)
{
	relocate_flag = false;
}

void radar_sensor::imu_rst()
{
