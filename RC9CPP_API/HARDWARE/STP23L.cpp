...
STP23L::STP23L(UART_HandleTypeDef *huart_) : SerialDevice(huart_)
{
}
    ...
    if (PACK_FLAG == PACK_GET_DISTANCE && state_flag == 0) /* 获取一帧数据并校验 */
    {
        if (state > 9)
        {
            ...
            if (state == 190)
                ...
                timestamp = ((u32)temp_data << 16) + timestamp, state++, crc = crc + temp_data;
            else if (state == 193)
                timestamp = ((u32)temp_data << 24) + timestamp, state++, crc = crc + temp_data;
            else if (state == 194)
            {
...
void STP23L::CLR_Buf(void)
{
    memset(USART_RX_BUF, 0, USART_REC_LEN); // 清空
    point1 = 0;
}

void STP23L::data_process(void)
{
    /* 计算距离 */
    static u8 cnt = 0;
    u8 i;
