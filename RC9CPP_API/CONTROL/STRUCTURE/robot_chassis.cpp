...
void RoboChassis::swerve3_calc(Vector2D robovel, float w)
{
    ...
    // 第二个舵轮
    target.swerve_motor_target[1].x = robovel.x + w * 0.144516f; // 0.150155=0.2451*cos(52.22°)
    target.swerve_motor_target[1].y = robovel.y - w * 0.197963f; // 0.193719=0.2451*sin(52.22°)
    ...
    // 第三个舵轮
    target.swerve_motor_target[2].x = robovel.x + w * 0.144516f; // 0.150151=0.2451*cos(52.22°)
    target.swerve_motor_target[2].y = robovel.y + w * 0.197963f; // 0.193719=0.2451*sin(52.22°)
