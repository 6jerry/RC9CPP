...
public:
    ...
    float Kp = 0.0f, Kd = 0.0f, wc = 0.0f, u = 0.0f, u0 = 0.0f, e1 = 0.0f, e2 = 0.0f;                                                  // LF用的参数

    float output_limit = 0.0f;
    uint32_t previous_time = 0;
    ladrc(float r_, float wc_, float w0_, float b0_, float output_limit_);
    void ladrc_SetParameters(float r_, float wc_, float w0_, float b0_);
