...
demo test2, test3;
go1can go1(0, &hcan1, 2.36f, 4.336f, 0.0012f); // 0.017f, 0.000024f, 0.00001f
...
r2n_setup(void)
{
    ...
    task_core.registerTask(0, &go1);
    // task_core.registerTask(3, &r2n_chassis);
    task_core.registerTask(0, &r2_remote);
...
void demo::process_data()
{
    ...
    data_chain.tx_frame_mat.data.msg_get[1] = go1.show_speed;
    data_chain.tx_frame_mat.data.msg_get[2] = go1.speed_pid.output;

    go1.speed_pid.superPID_SetParameters(data_chain.rx_frame_mat.data.msg_get[0], data_chain.rx_frame_mat.data.msg_get[1], data_chain.rx_frame_mat.data.msg_get[2]);
    // go1.I_cc = data_chain.rx_frame_mat.data.msg_get[0];
    go1.fff = data_chain.rx_frame_mat.data.msg_get[3];
    go1.icc = data_chain.rx_frame_mat.data.msg_get[4];
}