...
void process_udp_data(int sock) {
    ...
    if (len > 0) {
        ...
        if(rx_buffer[0] == '0' && rx_buffer[1] == ',' ) { // 接收到的是数据帧
            // 处理接收到的数据
            uint8_t len = rx_buffer[5] + 8;
            uart_write_bytes(UART_NUM_1, (const char*)(rx_buffer+2),  len);
            ESP_LOGI(TAG, "Received: %s,len = %d", rx_buffer+2,len);
        }

        else if(rx_buffer[0] == '2' && rx_buffer[1] == ',' ) {
            // 计算接收间隔
            ...
            ESP_LOGI(TAG, "Received unknown data: %s", rx_buffer);
        }
...
bool udp_communication_test_loop(int sock, struct sockaddr_in *server_addr) {
    ...
    if (current_time - last_send_time >= 16000) { // 16ms
        if(update_xbox){
            static uint8_t data[40] = "2,"; // 预构造包头

            memcpy(data+2, &parser.current_frame.data_length, 1);
            memcpy(data+3, parser.current_frame.data, parser.current_frame.data_length); // 仅需拷贝数据部分

            int sent_bytes = sendto(sock, data, 40, 0, (struct sockaddr *)server_addr, sizeof(*server_addr));
            if (sent_bytes < 0) {
