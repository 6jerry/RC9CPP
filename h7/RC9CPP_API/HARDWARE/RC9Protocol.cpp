...
__attribute__((section("dma_buffer_section"), aligned(4))) uint8_t sendBuffer_[MAX_DATA_LENGTH_RC9 + 8] = {0}; // 发送缓冲区，放到 AHB SRAM 的 DMA 段
// 实现获取待发送的数据
void RC9Protocol::process_data()
{
    ...
    if (status == osOK)
    {
        ...
        uint8_t send_l = data_to_send.data_length + 8;
        // 发送数据
        if (type == uart)
        {
        ...
        {
            CDC_Transmit_HS(sendBuffer_, data_to_send.data_length + 8);
        }
    }
    // 如果队列为空，返回并等待下次处理
}
