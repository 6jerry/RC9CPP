...
CanDevice::CanDevice(CanDeviceType deviceType_, FDCAN_HandleTypeDef *hcan_, uint8_t can_id) : deviceType_(deviceType_), hcan_(hcan_), can_id(can_id)
{
    // 注册该can设备
    if (hcan_ == &hcan1)
    {
...
HAL_StatusTypeDef CanDevice::CAN_Send(uint32_t can_id, uint8_t is_extended, uint8_t data[8])
{
    ...
    return HAL_CAN_AddTxMessage(hcan_, &txHeader, data);
}
